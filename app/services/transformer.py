...
def process_and_save(data: Iterable[dict], resource_name: str) -> str | None:
    ...
    # Add a filename column so CSV schema matches DB rows when desired.
...
def build_order_details_rows(
    order_details: List[Dict[str, Any]],
) -> List[Dict[str, Any]]:
    """Flatten order-detail payloads (with `lineItems`) into line-level rows.

    Each output row corresponds to a single line item, with order-level
    metadata repeated on each row. Orders with no `lineItems` still
    produce one row so that they are not lost.
    """
    rows: List[Dict[str, Any]] = []
...
def process_and_save_order_details(
    ...
    """Transform order-detail payloads into line-level rows and save to CSV."""
    if not order_details:
        return None

    rows = build_order_details_rows(order_details)
    ...
    # Keep a filename column consistent with the *_with_filename variant
    # so daily vs full ETL order-details CSVs share the same headers.
    df["filename"] = os.path.basename(filename)
    df.to_csv(filename, index=False)
    return filename
